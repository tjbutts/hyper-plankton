...
# Plot N:P ratio of nutrient stocks in Green Valley Lake # 
windows(height= 10, width = 10) # need to adjust 
par(mfrow=c(2,1),mai=c(0.1,1,0.1,1)) # need to adjust 
plot(gvl19$doy, log10(gvl19$org_NP), type = "o", lwd = 3, yaxt='n', xlim = c(120, 280), 
     pch = 15, col = "gray30", ylim = c(log10(0.1), log10(1000)),
     ylab = "", xlab = "", cex = 3, cex.lab = 1.5, cex.axis = 2, col.axis = 'white')
text(125, log10(200), "Phosphorus \nLimited", srt = 90, cex = 1.5)
text(125, log10(1), "Nitrogen \nLimited", srt = 90, cex = 1.5)
mtext('log10(N:P)', side = 2, line = 3, cex = 1.5)
...
zp_cnp = zp_stoich %>% rename(perc_c = `%C`) %>% # COMMENT OUT # 
  rename(perc_n = `%N`) %>%
  rename(perc_p = `%P`) %>%
  as_tibble()
...
zp_stoic_sum = mutate(zp_stoic_sum,zp_np = (nstorage)/(pstorage)) %>% arrange(doy)
zp_stoic_sum

par(mai=c(0.9,1,0.1,1)) # NEED TO ADJUST FORMATTING 
plot(zp_stoic_sum$doy, zp_stoic_sum$zp_np, type = 'o', xlim = c(120, 280),
     pch=18, col='black', cex=3, cex.axis = 2, lwd=3, xlab='', ylab='') 
...
par(mfrow=c(2,1),mai=c(0.1,1,0.1,1))
plot(gv_uML_stocks$doy, log10(gv_uML_stocks$totalN_uML+1),  type = "o", lwd = 3, yaxt='n', xlim = c(120, 280), 
     pch = 19, col = '#004ebe', ylim = c(log10(1), log10(500)),ylab='',
     #ylab = expression(N~Concentration~"("*mu*M*")"), 
     xlab = "", cex = 3, cex.axis = 2, cex.lab = 1.5,
     col.axis='white')
axis(side=2,
...
# Nutreint stocks to fit with the N:P figure style #====================
# '#004ebe','#2a81ff', '#94c0ff', 'gray60'

gvl19 # nutrient stocks without zoops 
zoop_stoich = zp_stoic_ugL %>% 
  group_by(doy) %>%
  summarise(ugp_L = sum(ugp_L), 
            ugn_L = sum(ugn_L), 
            ugc_L = sum(ugc_L)) %>%
  ungroup() %>%
  mutate(mgn_L = ugn_L/1000)
zoop_stoich

# Nitrogen Stocks 
windows(height= 6, width = 10)
par(mai=c(0.9,1,0.6,1))
plot(gvl19$doy, log10(gvl19$TN_mgL+1), type = "o", lwd = 3, yaxt='n', xlim = c(120, 280), 
     pch = 19, col = '#004ebe', ylim = c(log10(1), log10(10)),
     ylab = expression(N~Concentration~"("*mg~L^-1*")"), xlab = "Day of Year, 2019", cex = 3, cex.axis = 2, cex.lab = 1.5)
points(gvl19$doy, log10(gvl19$orgN_mgL+1), 
       type = "o", lwd = 3, col = '#2a81ff', pch = 19, cex = 3)
points(gvl19$doy, log10(gvl19$NOx_mgL+1), 
       type = "o", lwd = 3, col = '#94c0ff', pch = 19, cex = 3)
points(zoop_stoich$doy, log10(zoop_stoich$mgn_L+1), 
       type = 'o', lwd = 3, col = 'gray60', pch = 19, cex = 3)
axis(side=2,
     at=c(log10(0.1), log10(0.2), log10(0.3), log10(0.4), log10(0.5), log10(0.6), log10(0.7), log10(0.8), log10(0.9), log10(1),
          log10(2),log10(3),log10(4),log10(5),log10(6),log10(7),log10(8),log10(9),log10(10)), #Where the tick marks should be drawn
     labels = c('0.1','','','','','','','','', '1', '', '','','','','','','','10'), las=2)

# Legend
windows(height=5, width=5)
par(mai=c(0.9,1,0.6,1))
plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
legend("center", legend =c("Total N", "Organic N", "Inorganic N", "Zooplankton N"), 
       pch=19,
       pt.cex=3, cex=1.5, bty='n',
       col = c('#004ebe','#2a81ff', '#94c0ff', 'gray60'))
...
windows(height= 6, width = 10)
par(mai=c(0.9,1,0.6,1))
plot(gvl19$doy, log10(gvl19$TP_ugL+1), type = "o", lwd = 3, yaxt='n', xlim = c(120, 280), 
     pch = 19, col = '#c24ad7', ylim = c(log10(1), log10(500)),
     ylab = expression(P~Concentration~"("*mg~L^-1*")"), xlab = "Day of Year, 2019", cex = 3, cex.axis = 2, cex.lab = 1.5)
points(gvl19$doy, log10(gvl19$orgP_ugL+1), 
       type = "o", lwd = 3, col = '#d786e4', pch = 19, cex = 3)
points(gvl19$doy, log10(gvl19$SRP_ugL+1), 
       type = "o", lwd = 3, col = '#ebc3f2', pch = 19, cex = 3)
points(zoop_stoich$doy, log10(zoop_stoich$ugp_L+1), 
       type = 'o', lwd = 3, col = 'gray60', pch = 19, cex = 3)
axis(side=2,
     at=c(log10(0.1), log10(0.2), log10(0.3), log10(0.4), log10(0.5), log10(0.6), log10(0.7), log10(0.8), log10(0.9), log10(1),
          log10(2),log10(3),log10(4),log10(5),log10(6),log10(7),log10(8),log10(9),log10(10), log10(20), log10(30), log10(40), log10(50),
          log10(60), log10(70), log10(80), log10(90), log10(100), log10(200), log10(300), log10(400), log10(500)), #Where the tick marks should be drawn
     labels = c('0.1','','','','','','','','', '1', '', '','','','','','','','10', '', '','','','','','','','100','', '','','500'), las=2)

# Legend 
windows(height=5, width=5)
par(mai=c(0.9,1,0.6,1))
plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
legend("center", legend =c("Total P", "Organic P", "Inorganic P", "Zooplankton P"), 
       pch=19,
       pt.cex=3, cex=1.5, bty='n',
       col = c('#c24ad7','#d786e4','#ebc3f2', 'gray60'))
