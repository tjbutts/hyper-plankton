...
excretion_mass = join2
excretion_mass # Excretion in ug/L, SRP in ug/L, NOx in mg/L 

# Phosphorus (in log scale to increase visual differences) 
lowerbound_p = c(excretion_mass$Plow)
upperbound_p = c(excretion_mass$Phigh)
doy = c(excretion_mass$doy)
x = c(doy, rev(doy))
y = c(lowerbound_p, rev(upperbound_p))
plot(excretion_mass$doy, log(excretion_mass$SRP_ugL+1), type='l', xlim=c(140,260), col='white',
     cex.axis=1.1, ylim=c(log(1), log(220)), yaxt='n')
axis(side=2,
     at=c(log(1),
     ...
     labels = c('1','','','','','','','','','10', '', '','','','','','','','100','','300'), las=2, cex.axis=1)
#lines(excretion_frac$doy, lowerbound_p, type='l')
polygon(x=x, y=log(y+1), col=pcol, border=F)
lines(excretion_mass$doy, log(excretion_mass$Pexc+1), type='l', lwd=3, col='orchid3')
lines(excretion_mass$doy, log(excretion_mass$SRP_ugL+1), type='l', lwd=3, col='gray30')
legend('topleft', legend=c('Zooplankton excreted P', 'Inorganic P in surface waters'), 
       pch=15, col=c('orchid3', 'gray30'), cex=1.2)
# Axis text # 
mtext(side=1, 'Day of Year, 2019', line=2, cex=1.1 )
mtext(side=2, 'Inorganic P concentration (micrograms per liter)', line=2.5, cex=1.1)
